...
load_css("trendingbuzz/hidden_gems.css")
...
user_prompt = st.text_input("ANY thing entertaiment:")
...
if 'generate_query' not in st.session_state:
    st.session_state.generate_query = False  # Set it to False initially
...
def generate_query():
    ...
    try:
        ...
        if results:
            movies = [{'title': row.title, 'reviewer_rating': row.reviewer_rating} for row in results]
            st.session_state['movies'] = movies
            st.session_state['movies_csv'] = pd.DataFrame(movies).to_csv(index=False)
        else:
            st.session_state['movies_csv'] = ''
            st.error("No movies found with a rating of 6.0 or higher.")
    except Exception as e:
...
if st.button("Press to generate 10 random movies (6.0+ rating)", on_click=generate_query):
    if st.session_state.movies:
        for movie in st.session_state.movies:
            ...
            # Assuming 'imagen' is a defined function to generate and display movie posters
            imagen(f"A movie poster for {movie['title']}")  
        st.download_button(label="Download Movies Data as CSV", data=st.session_state.movies_csv, file_name="movies_data.csv", mime='text/csv')
    else:
        st.error("No movies generated yet. Please press the button to generate movies.")

st.markdown("---")
st.markdown("© 2024 KEM ENTERTAINMENT. All rights reserved.")
